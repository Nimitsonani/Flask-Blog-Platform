...
class BlogPost(db.Model):
    ...
    poster_id = db.Column(Integer,db.ForeignKey('Users.id'))
    comments = db.relationship('Comment',backref='comments_blog')
...
class User(UserMixin,db.Model):
    ...
    comments = db.relationship('Comment',backref='comments_user')
...
class Comment(db.Model):
    ...
    parent_post_id = db.Column(Integer,db.ForeignKey('blog_posts.id'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash('No User Found. Try Register Instead.')
...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts,is_admin=admin_logged_in)
...
def show_post(post_id):
    ...
    comment_on_the_post=db.session.execute(db.select(Comment).where(Comment.parent_post_id==post_id)).scalars()
    return render_template("post.html", post=requested_post,form=form,is_admin=is_admin,comments=comment_on_the_post,gravatar=gravatar)
